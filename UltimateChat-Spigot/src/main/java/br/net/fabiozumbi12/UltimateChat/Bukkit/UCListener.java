        ...
        if (sender instanceof Player) {
            ...
            if (cmd.getName().equalsIgnoreCase("uchat")) {
                if (args.length == 1) {
                    ...
                    if (args[0].equalsIgnoreCase("spy")) {
                        ...
                        if (!UChat.get().isSpy.contains(p.getName())) {
                            ...
                            UChat.get().getLang().sendMessage(p, "cmd.spy.disabled");
                        }
                        return true;
                    }
                }
                ...
                if (args.length == 2) {
                    ...
                    if (args[0].equalsIgnoreCase("addfilter")) {
                        ...
                        if (!args[1].contains(":")) {
                            UChat.get().getLang().sendMessage(p, "cmd.addfilter.invalid");
                            return true;
                        }
            ...
            if (UChat.get().getChAliases().contains(label.toLowerCase())) {
                if (args.length >= 1) {
                    ...
                    Bukkit.getScheduler().runTaskAsynchronously(UChat.get(), () -> UCMessages.sendFancyMessage(new String[0], msg, ch, sender, null));
                    return true;
                }
            }
        }
    ...
    public void onChat(AsyncPlayerChatEvent e) {
        ...
        if (UChat.get().tellPlayers.containsKey(p.getName()) && (!UChat.get().tempTellPlayers.containsKey("CONSOLE") || !UChat.get().tempTellPlayers.get("CONSOLE").equals(p.getName()))) {
            ...
            if (ch.isCmdAlias()) {
                ...
                UCMessages.sendFancyMessage(e.getFormat().split(","), e.getMessage(), fch, p, null);
                e.setCancelled(true);
            }
        }
    }
            ...
            if (args.length > 0) {
                for (String arg : args) {
                    if (Bukkit.getPlayer(arg) != null) {
                        if (!(sender instanceof Player) || ((Player)sender).canSee(Bukkit.getPlayer(arg)))
